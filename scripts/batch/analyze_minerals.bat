...
REM First, make sure the CSV file is accessible in the container
docker cp "ekspor_mineral_indonesia_WITS.csv" spark-master:/tmp/

echo Creating analysis script inside container...
docker exec -t spark-master bash -c "cat > /tmp/analyze_minerals.py << 'EOL'
...
docker exec -t spark-master python /tmp/analyze_minerals.py
